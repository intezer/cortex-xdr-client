...
class ActionsAPI(BaseAPI):
    ...
    def get_action_status(self, group_action_id: int) -> GetActionStatus | None:
        """
        ...
        response = self._call(call_name="get_action_status", json_value=request_data)
    ...
    def get_file_retrieval_details(
        self, group_action_id: int
    ) -> GetActionStatus | None:
        """
        ...
        response = self._call(
            call_name="file_retrieval_details", json_value=request_data
        )
        return GetActionStatus.model_validate(response.json())
